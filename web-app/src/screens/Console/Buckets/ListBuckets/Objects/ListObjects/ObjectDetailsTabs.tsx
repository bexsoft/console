...
import { Box, Button, PlusIcon, Tabs, Tag, useMDSTheme } from "mds";
import { AppState, useAppDispatch } from "../../../../../../store";
import { selDistSet } from "../../../../../../systemSlice";
import { IAM_SCOPES } from "../../../../../../common/SecureComponent/permissions";
import { setLoadingObjectInfo, setSelectedTab } from "../../../../ObjectBrowser/objectBrowserSlice";
import ConfirmDialog from "../../../../Common/ModalWrapper/ConfirmDialog";
import TagsModal from "../ObjectDetails/TagsModal";
import { calculateLastModifyTime, niceBytes, niceBytesInt } from "../../../../../../common/utils";
...
const ObjectDetailsTabs = ({
  ...
  const deleteTagProcess = async () => {
    ...
    /*const response = await setObjectTags({
      siteId: siteId!,
      namespace: namespace!,
      ...
      prefix: objectName,
      version_id: verID,
      minioSetObjectTagsRequestBody: { tags: cleanObject },
    });

    if (response.error) {
      // TODO: Enable Error Bars
            ...
            content: (
              ...
              >
                ...
                <Box className={"detailContainer"}>
                  ...
                  <div className={"SM_Strong"}>{niceBytes(`${actualInfo.size || "0"}`)}</div>
                </Box>
                {versions &&
                  totalVersionsSize &&
                  ...
                  selectedVersion === "" && (
                    <Box className={"detailContainer"}>
                      ...
                      <div className={"SM_Strong"}>{versions.length} version
                      {versions.length !== 1 ? "s" : ""},{" "}
                        {niceBytesInt(totalVersionsSize)}</div>
                    </Box>
                  )}
                {selectedVersion === "" && (
                  <Box className={"detailContainer"}>
                    <div className={"labelTitle SM_Normal"}>Last Modified:</div>
                    <div className={"SM_Strong"}>{calculateLastModifyTime(actualInfo.last_modified || "")}</div>
                  </Box>
                  )}
                <Box className={"detailContainer"}>
                ...
                <Box className={"detailContainer"}>
                  ...
                  >
                    <Fragment>
                      ...
                      <div className={"SM_Strong"}>{actualInfo.legal_hold_status ? "On" : "Off"}</div>
                    </Fragment>
                  </SecureComponent>
                </Box>
                <Box className={"detailContainer"}>
